...
function start-gardener() {
  ...
  $SCRIPTS/wait-for.sh seed local GardenletReady SeedSystemComponentsHealthy ExtensionsReady
}
...
function start-shoot() {
  ...
  # sleep $SLEEP &
  # wait
  NAMESPACE=garden-local $SCRIPTS/wait-for.sh shoot local APIServerAvailable ControlPlaneHealthy ObservabilityComponentsHealthy EveryNodeReady SystemComponentsHealthy
}
